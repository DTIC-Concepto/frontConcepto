...
export async function GET(
  ...
  try {
    ...
    const backendUrl = new URL(
      `${BACKEND_URL}/reportes/matriz-asignaturas-eurace/${carreraId}`
    );
    ...
    console.log('Llamando al backend:', backendUrl.toString());
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.error || errorData.message || 'Error al obtener matriz' },
    ...
    console.error('Error en API route matriz-asignaturas-eurace:', error);
    return NextResponse.json(
