...
export async function GET(request: NextRequest) {
  try {
    ...
    while (hasMore) {
      ...
      const backendUrl = `${BACKEND_URL}/carreras?${urlParams.toString()}`;
      ...
      if (!response.ok) {
        ...
        console.error('Backend error carreras:', response.status, errorText);
        return NextResponse.json(
          { error: 'Error al obtener carreras del backend' },
      ...
      if (data && Array.isArray(data.data)) {
        ...
        console.log(`Carreras página ${page - 1}: ${data.data.length} items, total acumulado: ${currentCount}/${total}`);
      ...
      // Límite de seguridad
      if (page > 100) {
        console.warn('Carreras: Límite de páginas alcanzado');
        break;
      }
    }

    console.log('Carreras GET successful: Total de', allData.length, 'carreras');
    ...
    console.error('Error en proxy de carreras:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/carreras`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    console.error('Error en proxy de carreras (POST):', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
