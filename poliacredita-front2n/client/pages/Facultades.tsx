    ...
    <div className="p-4 md:p-6">
      ...
      <div className="bg-white rounded border border-border p-4 mb-6 flex flex-col md:flex-row items-stretch md:items-center gap-4">
        ...
        <div className="flex items-center gap-1.5 px-3 py-2 border border-border rounded bg-white md:min-w-[180px]">
          <span className="text-sm flex-1 truncate">Filtrar por Carreras</span>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#F3F4F6]">
                <th className="text-left px-4 py-3.5 text-sm font-normal text-muted">Código</th>
                <th className="text-left px-4 py-3.5 text-sm font-normal text-muted">Nombre</th>
                <th className="text-left px-4 py-3.5 text-sm font-normal text-muted">Carreras</th>
                <th className="text-left px-4 py-3.5 text-sm font-normal text-muted">Decano</th>
                <th className="text-center px-4 py-3.5 text-sm font-normal text-muted">Acciones</th>
              </tr>
            </thead>
            <tbody>
              {facultiesData.map((faculty, index) => (
                <tr key={index} className="border-t border-border">
                  <td className="px-4 py-4 text-sm text-foreground">{faculty.code}</td>
                  <td className="px-4 py-4 text-sm text-foreground">{faculty.name}</td>
                  <td className="px-4 py-4 text-sm text-foreground">{faculty.careers}</td>
                  <td className="px-4 py-4 text-sm text-foreground">{faculty.dean}</td>
          ...
          <div key={index} className="bg-white rounded border border-border shadow-sm p-4">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <div className="font-medium text-foreground">{faculty.code}</div>
              </div>
            ...
            <div className="mb-2">
              ...
              <div className="text-sm text-foreground">{faculty.name}</div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-xs text-muted mb-1">Carreras</div>
                <div className="text-sm text-foreground">{faculty.careers}</div>
              </div>
              <div>
                <div className="text-xs text-muted mb-1">Decano</div>
                <div className="text-sm text-foreground">{faculty.dean}</div>
              </div>
            </div>
          </div>
        ))}
